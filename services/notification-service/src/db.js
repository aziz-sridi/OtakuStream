...
const db = (async () => {
  ...
  if (fs.existsSync(DUMP_FILE)) {
    try {
      ...
      console.log('[notification] restored rxdb state from', DUMP_FILE);
    } catch (e) {
      console.warn('[notification] could not restore rxdb dump:', e.message);
    }
  }
  ...
  const flush = async () => {
    try {
      ...
      console.warn('[notification] rxdb flush failed:', e.message);
    }
  };
